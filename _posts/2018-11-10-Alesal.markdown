...
Alesal Engine is a multi threading game engine written in C++ and OpenGL. It has been developed by two students of Video Games programing at ESAT, to test what we can achieve for a subject, and it can be used to assemble scenes using LUA language :)
...
## Render
To render, the engine has some differents capabilities, the most important one is that everything, is being rendered using deferred pipeline. That means that we have several textures we can get by rendering the scene several times to get the information we need to draw the scene as is currently seen.
The way we render every object is using what we called renderBuckets, a collection of draw commands on a specific order, we have several of them, for example one for opaques, another for occlusion textures, another for directional shadows… and a lot of more…
...
The directional light just create a simple depth texture from the point of view of the own light.
...
Just comparing the depth of the current pixel, with the transpolated depth of the light.
The brief version would be that if the pixel depth is lower than the generated on the shadow map, this pixel is affected by the light, but if is higher is occluded.

![ShadowScreenShot](/assets/Alesal/Screenshot_3.png)


## Occlusion Culling
To be able to create a occlusion culling method, we have take profit of the open gl queries, so when we are going to render something, we ask if we were visible on the last frame.
if we were visible, at least 1 pixel, we can be rendered, but if not, we are not going to be rendered.


## Pipeline
The pipeline of our engine consist on several passes, where we just store the information we are going to use on a separated light pass.
...
* The Uvs of the texture is going to use (trickied pbr ) 
* Normal tangent (same last, a trick to PBR)

If the object needs or not to be bloomed:
* The load of specular the drawable has.
...
## Materials
We have support for some different built-in materials, PBR, using differents textures to get the information, or just an albedo texture and several floats to tweak the look we want.
...
This material has almost all the basic info, and a way of generate new textures and some ways to tweak minor values.

We have a PBR support material, using 5 textures.
Albedo: Where the base color of the surface is specified.
Normal: A fake illusion show a bumpier surface than what really is.
...
AO: To simulate some shadows, a tricky texture that allow the artists to generate whether the light is more likely to escape from the theoretical material.

We have the same material, but using values to represent the whole value of roughness and metallness, to be able to tweak some different materials, without the need of a new texture per each one.
...
## SSAO
Is the most trickiest effect we have done, and the most heaviest effect we have.
...
The key is to calculate a occlusion factor based on how many object are surrounding the one we are currently checking.
The value we obtain from this operation is the one that can modify the ambient factor we use later on the lightpass.
...
Most of what we learnt was thanks to: 
